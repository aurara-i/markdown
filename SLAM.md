...
提到批量状态估计问题可以转换为最大似然估计问题，并使用最小二乘法进行求解。<br>
     - **贝叶斯公式**
<img width="601" height="156" alt="图片" src="https://github.com/user-attachments/assets/a162de65-6bf4-4d2b-b555-5acf02b152f2" /><br>
...
<br>**上面这部分是进行预测的**
<br>至此，我们给出了贝叶斯估计，因为上式还没有具体的概率分布形似，所以没法实际操作它。对这一步的后续处理，方法上产生了一些分歧。大体上讲，存在下面两种选择：<br>
     - 1、**马尔可夫性**：简单的一阶马氏性认为，k时刻的状态只和k-1时刻有关，我们会获得扩展卡尔曼滤波（EKF）为代表的滤波器方法。<br>
         - 1、线性系统 KF<br>
         - 上面状态转移矩阵转化成：P(xk​∣xk−1​,uk​)，当前状态 𝑥𝑘只与前一时刻状态 𝑥𝑘−1有关，与更早的状态 𝑥0,𝑥1无关。而且，在状态转移时刻，系统的控制输入𝑢𝑘是外部已知的影响量。而且没有Z，只需要知道上一个状态和当前控制量就够了，之前的历史信息不会再直接影响当前状态。<br>
         - 上面上一时刻的式子转化成：P(xk−1​∣x0​,u1:k​,z1:k−1​)=P(xk−1​∣x0​,u1:k−1​,z1:k−1​)，预测上一步的状态时，只需要之前的控制量和观测，不可能受到未来的输入影响。<br>
         - 计算后验概率：https://chatgpt.com/s/t_68f99d8c40848191b1bb6e8e8c82b979 通过这个式子获得卡尔曼增益K，让背后获得更新后的概率和情况。<br>
     - 2、考虑k时刻和前面所有状态之间的关系，获得以**非线性优化**为主题的优化框架。<br>
        - **非线性系统和EKF** 因为一般情况下运动和观测方程都是非线性函数，一个高斯函数经过非线性变化后也不再是一个高斯分布。<br>
        - 做法是：运动方程和观测方程一阶泰勒展开，只保留一阶项，然后按照线性系统进行推导.<br>
        - EKF局限：1、在一定程度上假设马尔可夫性，无法处理和很久之前相关的数据。2、非线性误差：仅仅做了一次线性化，不一定能够近似整个函数。3、不适合大型场景，储存均值方差，数据量大。4、没有异常值检测机制。<br>
     - **BA与图优化**
         - 原理和前面BA优化基本原理相同，只是EF现在是整体函数对整体变量的导数。
        <img width="549" height="420" alt="image" src="https://github.com/user-attachments/assets/76383994-5955-4b17-9c18-beb5e07b8ff4" />
 <br><img width="583" height="222" alt="image" src="https://github.com/user-attachments/assets/c77e158d-0c4a-4002-b96b-dab0b0393624" />
           <br><img width="609" height="320" alt="image" src="https://github.com/user-attachments/assets/856f4784-7a2c-4fc8-81f2-1ce16ee96255" />
<br><img width="705" height="405" alt="image" src="https://github.com/user-attachments/assets/984caf48-323c-4875-a5d8-ce412d05f340" />
<br><img width="664" height="555" alt="image" src="https://github.com/user-attachments/assets/f7caf5ce-272d-4999-b1c2-f3d17a242027" />


         - **稀疏矩阵和图优化**
 <br>SLAM（同时定位与建图）要同时求出：相机的轨迹（姿态参数）路标点的位置（地图）所以问题是一个巨大的最小二乘问题：min ∑𝑖,𝑗∥𝑓(𝐶𝑖,𝑃𝑗)−𝑧𝑖𝑗∥2每一项 𝑓(𝐶𝑖,𝑃𝑗)表示“第 𝑖 个相机看到第 j 个路标点”的误差。这么说明这个观测便之和几个顶点有关。
<br>为什么海森矩阵是稀疏的？这个误差只跟「一个相机」和「一个路标」有关，跟别的变量无关。所以，它的雅可比矩阵（Jacobian）大多数位置都是零。因为雅可比矩阵 J是「大部分为零」的，所以乘出来的 H 也是「大部分为零」的。非零块只出现在「有关联的变量之间」。
<br>边缘化（Marginalization）不是图优化本身，而是图优化中的一个数学步骤或技巧.它是 在图优化框架下，用来消除部分变量、减小问题规模 的一种方法。图优化是“大框架”；边缘化是“局部手段”。
         - **鲁棒核函数**
 <br>核函数保证每条边的误差不会太大而掩盖其他边，具体方式是把原先误差的二范数度量替换成一个增长没那么快的函数，同时保证自己的光滑性质，是整个优化过程更为稳健。
<br><img width="648" height="166" alt="image" src="https://github.com/user-attachments/assets/58013a10-7b4d-443f-aaab-3b4d03e8c278" />
**后端2**
     - 滑动窗口滤波和优化
     -  位姿图：大量的特征点会降低计算效率，增大计算量以至于无法实时化，利用位姿图进行简化
 <br>滑动窗口法：BA固定在一个时间窗口内，离开窗口则被丢弃。
 <br>在滑动窗口中，当窗口结构发生变化状态变量会如何变化：1、新增关键帧和它可能观测到的路标点。2、删除关键帧和观测到的路标点。 新增的时候只需要按照正常的BA流程即可，但是在删除旧的关键帧的时候它并不是孤立的，x1边缘化之后会导致整个问题不再稀疏。
<br>边缘化是为了在优化中“去掉不再需要的变量”，保留这些变量的统计约束信息（schur），从而实现 计算量与精度的平衡。
<br>原本的系统是“稀疏”的：
每个路标点只与被它观测到的相机帧相连；不同路标点之间互相独立，优化计算很快。
但是——当我们边缘化掉一个与多个路标点相连的关键帧时，这些路标点之间产生了先验信息（prior），也就是“虚连接”（dense coupling）。👉 这会破坏稀疏性！
所以右图那一大片黑色区域表示：边缘化操作让原本稀疏的信息矩阵变得更稠密了
 <img width="730" height="844" alt="image" src="https://github.com/user-attachments/assets/1649ee5d-0a22-40c2-9252-505a2aa72089" />

    - 位姿图的优化 
    - 

<br>**回环检测**<br>
     - 随着路径的不断延伸，机器人的建图过程会存在不断地累计误差。现在最为常用消除累计误差的方法就是利用回环检测来优化位姿。
     - 回环检测：又称闭环检测，是指机器人识别曾到达某场景，使得地图闭环的能力。说的简单点，就是机器人在左转一下，右转一下建图的时候能意识到某个地方是“我”曾经来过的，然后把此刻生成的地图与刚刚生成的地图做匹配。回环检测之所以能成为一个难点，是因为：如果回环检测成功，可以显著地减小累积误差，帮助机器人更精准、快速的进行避障导航工作。而错误的检测结果可能使地图变得很糟糕。因此，回环检测在大面积、大场景地图构建上是非常有必要的 。
     - 1、能够为后端位姿图提供更多有效的数据。2、进行重定位3、长时间下的正确性。
     - 1、哪里会出现回环的问题？基于里程计的几何关系（需要进行假设存在逻辑倒置的问题）和基于外观的几何关系（只根据外观，即图像相似性）。还有室外GPS确定。
         - 基于外观的讨论：如何判断图像的相似性？评判相似程度，引入感知偏差和感知变异的概念。
         <br><img width="354" height="122" alt="image" src="https://github.com/user-attachments/assets/b35c895f-2904-430d-bf8e-f5df558cfca0" />
         <br>这里的假阳性称之为感知偏差，假阴性·称之为感知变异。真阳性我们用TP表示，真阴性我们用TN表示，所以我们希望TN TP高，FN FP尽可能低。我们计算两个统计量：准确率和召回率。
         <br><img width="490" height="54" alt="image" src="https://github.com/user-attachments/assets/dacbbd8f-7064-4646-afc5-a47c972ca4c9" />
         <br>准确率是算法中提取的所有回环是真的概率。召回率是指，所有真实回环呗检测出来的概率。SLAM中我们要求准确率要优于召回率。
     - 2、词袋模型
         - <img width="788" height="410" alt="image" src="https://github.com/user-attachments/assets/33cd3bde-1a05-4bfb-bdc9-079a227ae81a" />
     - 3、字典
         - 字典是由很多单词组成，每个单词代表一个概念，是一类特征的组合。字典问题就类似于一个聚类。（无监督机器问题常见，让及其自行寻找数据中的规律）。
         - K-means算法寻找字典：1、随机选取K个中心点 2、对于每个样本，计算与中心点的距离，取最小的作为聚类。3、重新计算每个类的中心点。4、如果每个中心点变化都很小则收敛。这样成功把大量特征点聚类成k个单词的字典。
         - K叉树表示字（kmeans扩展）：1、根节点kmeans把样本聚成k类为第一层。2、对第一层的每个节点，吧属于该节点的样本再聚成K类3、以此类推
     - 4、相似度计算：
         - 我们希望对单词进行区分，区分不同单词的重要性：TF-IDF（译频率-逆文档频率）。TF认为单词在图像中经常出现，区分度就高。IDF认为单词出现的频率越低，分类时区分度越高。TF 关注的是某个词（或视觉特征）在一张图片内部的重要性。如果某个单词（或局部特征）在当前图片中频繁出现，说明它在这张图片中代表性强。IDF 关注的是该词在整个数据集中是否具有区分性。如果一个词在所有图片里都常见（比如“背景”、“边缘”），它对分类没用。如果一个词只在少数类中出现（比如“树叶”只出现在森林类），那它能更好地区分类别。权重w=TF*IDF

<br>**建图**<br>
     - 地图的作用：定位（稀疏地图）、（导航、避障、重建）（需要建立稠密地图）、交互（语义地图）。
     - 稀疏地图是特征点的建模。稠密地图是所有见过的地方。
     - **单目稠密重建**
         - 单幅像素只能提供角度和亮度。在稠密重建中，我们需要的值每一个像素的距离，解决方案：1、使用单目相机，估计相机运动，利用三角化计算像素距离。2、使用双目相机，利用左右目的视差计算像素的距离。3、利用RGB-D直接获得像素距离。1、2称之为立体视觉（计算量大，可靠性不高）。RGB-D无法很好的应用在室外。
         - 如何确定某像素出现在其他图里的位置？极线搜索与块匹配。然后如何确定深度？高斯分布的深度滤波器技术。
         - 1、什么是极线搜索？背景：我们有两个相机，分别从左右两个视角拍摄同一个场景。左边的相机看到了一个像素点 P1​，但我们不知道它在三维空间中的具体位置（即不知道它离相机多远）。问题：我们不知道这个点的深度（距离），因此它在空间中可能的位置有很多，形成了一条从相机出发的射线。在右边的相机图像上，这条射线会投影成一条线，叫做极线。核心思想：我们不知道 P1​ 在极线上的具体位置，只能沿着p2的极线逐个尝试，看看哪个点最像左边的 P1​。
         - 2、为什么单个像素比较不可靠？图像中可能有很多像素点的亮度值和 P1​ 相似，导致我们无法确定哪个是正确的匹配点。单个像素容易受到噪声、光照变化的影响，稳定性差。
         - 3、块匹配？为了提高匹配的可靠性，我们不再只比较单个像素，而是比较一个小区域（称为“块”或“窗口”）。具体做法：1、在左边的图像中，以 P1​ 为中心，取一个大小为 w×w 的小方块（比如 7×7 或 11×11 像素）。2、 在右边图像的极线上，也取很多同样大小的小方块。3、比较左边的小方块和右边极线上每个小方块的相似程度。  如何计算3中小块之间的差异？1、SAD	绝对差之和	两个小块对应像素差值的绝对值之和	数值越小越相似。2、SSD	平方差之和	两个小块对应像素差值的平方和	数值越小越相似。3、NCC	归一化互相关	计算两个小块的归一化相关性	数值越接近1越相似。/我们可以先把每个小块的像素值减去各自的平均值，再进行上述计算。这样做的好处是：即使两个小块整体亮度不同（一个亮一点，一个暗一点），但仍然可能被认为是相似的。
         - 高斯分布的深度滤波器：

<br>**设计SLAM系统**<br>
     - 工程框架：1、bin下存储编译好的二进制文件2、include/myslam放置头文件3、src放置源代码4、test放测试用的文件5、config放置配置文件6、cmake_modules放置cmake文件。
     - <img width="811" height="369" alt="image" src="https://github.com/user-attachments/assets/83a7c86e-607c-4e14-9e07-2ed2aa344afb" />
     - <img width="797" height="150" alt="image" src="https://github.com/user-attachments/assets/9e49aa41-0c98-45e4-a6c2-cb2c0c2ddba9" />
