...
<br>**n系速度更新算法:**
    ...
    慢变项简化：重力哥氏积分项因变化平缓，用 “中间时刻外推 + 梯形积分” 快速计算；
    快变项精细：比力积分项因姿态 / 比力变化快，需分解姿态矩阵、补偿旋转 / 划桨效应，消除离散化误差；
    数据依赖：中间时刻的位置 / 速度需用前两帧外推（无当前数据时的最优估计）；
    最终合并：两项积分结果叠加到历史速度上，得到当前速度 —— 为后续 “位置更新” 提供关键输入。
...
<br>2、速度更新中为什么要先只加旋转效应和双子羊然后投影到n系再加上重力项，公式里不是加上旋转双子羊和重力项吗？
因为加速度计测到的 dvel是在机体系下的，而且机体系在积分过程中是不断旋转的，所以需要用 旋转效应（sculling 和 coning 补偿） 去修正，然后再把结果投影到导航系。重力和科氏力项本来就是在导航系下定义的，不需要额外旋转，只要直接积分到导航系速度就行。
<br>1. 输入数据:
<br>历史数据
...
<br>第一步 速度更新  速度是位置和姿态的基础，需先通过 IMU 线加速度增量，补偿地球运动（重力、哥氏力）和 IMU 误差（旋转、划桨效应），得到当前速度。旋转与划桨补偿公式本身就是基于 整步积分展开推导出来的；半步处理只用于外推位置和速度，以便计算中点的地理参数，不作用于 IMU 增量补偿。
<br>第二步 位置更新算法  位置更新并不是直接用 vk，而是用中间时刻的速度 vk−1/2
<br>第三步 姿态更新算法  在惯导姿态更新中，姿态变化微分方程需要通过数值积分实现，如果只用起点值做欧拉积分，精度只有一阶，误差会在积分中快速积累。通过在 **中间时刻 (k-1/2)** 重新计算位置、速度和地理参数，从而得到更接近真实平均值的角速度和参考系转动量，可以实现二阶精度的中点积分法，大幅减小截断误差。因此，姿态变换时引入中间时刻是为了提高积分精度、减小累积误差，保证长期导航解的稳定和可靠。
